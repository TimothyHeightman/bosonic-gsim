...
class NumericalResultTests(unittest.TestCase):
    def test_exact_reference_errors(self):
        self.assertLess(result("fig7_gaussian")["squeezing_max_fock_error"], 1e-10)
        self.assertLess(result("fig8_beyond_gaussian")["random_interferometer_max_fock_error"],
                        1e-10)
        self.assertLess(result("fig2_bounded_n")["fixed_sector_max_fock_error"], 1e-10)
        self.assertLess(result("fig3_doublon")["validation_max_fock_error"], 1e-10)
        self.assertLess(result("fig5_otoc")["validation_max_fock_error"], 1e-10)
        cubic = result("fig10_nilpotent_phase")
    ...
    def test_mixed_sector_readout_distinction(self):
        mixed = result("fig2_bounded_n")["mixed_sector"]
    ...
    def test_doublon_strong_coupling_power(self):
        exponent = result("fig3_doublon")["strong_coupling"]["fit_exponent"]
        self.assertLess(abs(exponent + 1.0), 0.15)

    def test_squeezing_reference_and_orders(self):
        data = result("fig4_squeezing")
    ...
    def test_nilpotent_phase_signals_are_nontrivial(self):
        data = result("fig10_nilpotent_phase")
    ...
    def test_gradient_and_qualified_boundary_claims(self):
        self.assertLess(result("fig9_kerr_control_2d")["gradient_max_absolute_error"],
                        1e-8)
        self.assertFalse(result("fig6_doublon_topology")["computed_invariant"])
        self.assertFalse(result("fig6_chiral_transport")["computed_invariant"])
